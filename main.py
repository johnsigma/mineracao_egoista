...
repeticoesPorDia = cria_repeticoes_por_dia(mineracoesDias)
df = cria_dataframe(repeticoesPorDia, 5)

# plota_grafico(df)

imprime_mineradores(df, 5)

maior_minerador = busca_minerador_mais_poderoso(df)

mineracoes_sequencia = conta_mineracoes_sequencia(
...
def teste_k_permutacoes(data, maior_minerador, k=1000):
    ...
    for i in range(k):
        ...
        mineracoes = conta_mineracoes_sequencia(novo_array, maior_minerador)
        mineracoes_sequencia.append(mineracoes)

    return mineracoes_sequencia


mineracoes_sequencia = teste_k_permutacoes(
    mineracoesDias, maior_minerador, 1000)

mineracoes_sequencia.sort(reverse=True)

print(mineracoes_sequencia)
